...
\[
  \p = (p(1), p(2), \ldots, p(\kappa)),\quad \sum_{i=1}^\kappa p_i = 1.
\]
...
Обозначим $\Pi[p, \sigma, \tau]$ вероятностное распределение на $S \times I^n \times J^n$, порожденное ходом случая и смешанными стратегиями $\sigma$ и $\tau$ игроков.
...
\begin{remark}
  ...
  Из равенств (\ref{ch1:eq:payoff-symmetry}) следует, что выигрыши игроков в играх $G^{m, \Co}_n(p)$ и $G^{m, \DCo}_n(1-p)$ при использовании соответственно стратегий $\sigma,\ \tau$ и $\symm{\sigma},\ \symm{\tau}$ совпадают.
\end{remark}
...
Таким образом, $\upperBound{p}$ является вогнутой кусочно-линейной функцией, график которой состоит из $m~+~1$ линейных сегментов.
...
\begin{figure}[thb]
  ...
  \begin{tikzpicture}[yscale=1.3,xscale=7]
    ...
    \node at (0.55, 2.6) {$H^m_\infty(p)$};
  \end{tikzpicture}
  \caption{График функции $H^m_\infty(p)$}
  \label{ch1:fig:Hm}
\end{figure}
...
\begin{lemma}
  ...
  Тогда при использовании вторым игроком стратегии $\tau^*$ в игре $\infiniteGame{p}$\textup{,} выигрыш первого игрока ограничен сверху величиной $\upperBound{p}$, т.е.
  \[
...
\begin{proposition}
  ...
  При использовании $\fGeneral{k}$ первый игрок обеспечивает себе на первом шаге выигрыш
  \begin{equation}
...
\begin{proof}
  ...
  \begin{align*}
    \firstPlayerPayoff{1}{p}{\fGeneral{k}}{j} 
    &=
      p \fGeneral[1,k]{H} (\Co (k+1) + \DCo k - m) +
      (1-p) \fGeneral[1,k]{L} (\Co (k+1) + \DCo k) = \\
    &= (p \fGeneral[1,k]{H} + (1-p) \fGeneral[1,k]{L}) (\Co (k+1) + \DCo k)
      - m p \fGeneral[1,k]{H} =\\
    &= q_k (\Co (k+1) + \DCo k) - m p(H|k) q_k  = \\
    &= (k + \Co - m p(H|k)) q_k.
  \end{align*}
  При $j>k+1$ получаем
  \begin{align*}
    \firstPlayerPayoff{1}{p}{\fGeneral{k}}{j} &=
    p \left( 
      \fGeneral[1,k]{H} (\Co j - \DCo k - m) +
      \fGeneral[1,k+1]{H} (\Co j - \DCo (k+1) - m)
    \right) + \\
    &+ (1-p) \left( 
      \fGeneral[1,k]{L} (\Co j + \DCo k) +
      \fGeneral[1,k+1]{L} (\Co j + \DCo (k+1))
    \right) = \\
    &= (\DCo k + \Co j)
    \left(
      p (\fGeneral[1,k]{H} + \fGeneral[1,k+1]{H}) +
    ...
    \right) - \\
    &- m p (\fGeneral[1,k]{H} + \fGeneral[1,k+1]{H}) +
    (p \fGeneral[1,k+1]{H} + (1-p) \fGeneral[1,k+1]{L}) \DCo = \\
    &= \DCo k + \DCo j - mp + \Co q_{k+1}.
  \end{align*}
...
Для $p = \pEven,\ k = \overline{1, m-1}$ определим $\fEven$ как распределение $\fGeneral{k}$ с параметрами
\begin{align*}
  q_k = \Co,& \quad p(H|k) = \pOdd[k-1] = (k-1+\Co)/m,\\
  q_{k+1} = \DCo,& \quad p(H|k + 1) = \pOdd[k] = (k+\Co)/m.
\end{align*}
...
Аналогично для $p = \pOdd,\ k = \overline{0, m-1}$ определим $\fOdd$ как распределение $\fGeneral{k}$ с параметрами
\begin{align*}
  q_k = \DCo,& \quad p(H|k) = \pEven[k] = k/\Co,\\
  q_{k+1} = \Co,& \quad p(H|k + 1) = \pEven[k+1] = (k+1)/\Co.
\end{align*}
Покомпонентно оно записывается как
\begin{gather*}
...
\begin{proposition}
  ...
  \begin{equation}
    ...
    \begin{gathered}
      \min_{j \in J}
      \firstPlayerPayoff{1}{\pEven}{\fEven}{j} = 0,\ k = \overline{0, m}, \\
      \min_{j \in J}
      \firstPlayerPayoff{1}{\pOdd}{\fOdd}{j} = \DCo\Co,\ k = \overline{0, m-1}.
    \end{gathered}
  \end{equation}
\end{proposition}
\begin{proof}
  ...
  Справедливость неравенства $\min_{j \in J} \firstPlayerPayoff{1}{\pEven}{\fEven}{j} \geq 0,\ k \in \{0, m\}$, то есть, соответствующего значению $p \in \{0, 1\}$, очевидна.
\end{proof}
...
Обозначим гарантированный выигрыш первого игрока в игре $\generalGame{n}{p}$ при применении стратегии $\fOpt$ через
\begin{equation*}
  \lowerBound[n]{p} = \min_{\tau \in \Tau} \firstPlayerPayoff{n}{p}{\fOpt}{\tau}.
\end{equation*}
Из утверждения~\ref{ch1:prop:K1-base} непосредственно следует, что
\begin{equation}
  \label{ch1:eq:L_1(p)}
  \begin{gathered}
  \lowerBound[1]{\pEven} = 0,\ k = \overline{0,m},\\
  \lowerBound[1]{\pOdd} = \Co \DCo,\ k = \overline{0,m-1}.
  \end{gathered}
\end{equation}
...
\begin{equation}
  ...
  \sum_{i \in I} q_i \firstPlayerPayoff{n-1}{p(H|i)}{\sigma(i)}{\tau(i)}.
\end{equation}

При использовании стратегии $\fOpt$ в силу \eqref{ch1:eq:L_1(p)} и \eqref{ch1:eq:recursive-structure} для гарантированного выигрыша первого игрока в игре $\generalGame{n}{p}$ справедливы формулы
\begin{equation}
  \begin{gathered}
...
\begin{proposition}
  ...
  \begin{gather}
    ...
    L_{2k+1} = \frac{(m - k - \Co)(k + \Co) + \DCo\Co}{2} = \upperBound{\frac{k+\Co}{m}}, \quad k = \overline{0, m-1},\\
    \label{ch1:eq:recurrence-solution:even}
    L_{2k} = \frac{k (m-k)}{2} = \upperBound{\frac{k}{m}}, \quad k = \overline{0, m}.
  \end{gather}
\end{proposition}
\begin{proof}
...
\begin{proof}
  ...
  Далее, пусть априорной вероятности $p_1 \in [0, 1]$ отвечает одношаговая стратегия $\sigma^1$ с параметрами $(q^1_i, p^1(H|i),\ i \in I)$, а априорной вероятности $p_2 \in [0, 1]$, отвечает стратегия $\sigma^2$ с параметрами $(q^2_i, p^2(H|i),\ i \in I)$.
...
\begin{remark}
  Одношаговая стратегия $\sigma^c$ из утверждения~\ref{ch1:prop:first:combination:step} не является линейной комбинацией одношаговых стратегий $\sigma^1$ и $\sigma^2$ в смысле покомпонентного равенства:
  \[
    \sigma^{c,H}_i \neq \lambda \sigma^{1,H}_i + (1-\lambda) \sigma^{2,H}_i,\ i \in I.
  \]
\end{remark}

Стратегию $\sigma^c$ из утверждения~\ref{ch1:prop:first:combination:step} можно определить для игры $\generalGame{n}{p},\ n \in \N$ произвольной продолжительности, рекурсивно продолжив ее применение на последующих шагах игры для соответствующих значений апостериорных вероятностей.

\begin{proposition}
  \label{ch1:prop:first:combination:game}
  При $\lambda \in (0, 1), \, k = \overline{0, m-1}$, и использовании первым игроком стратегии $\sigma^c$ для его гарантированного выигрыша в игре $\infiniteGame{p}$ справедливы равенства
  \begin{align*}
...
Таким образом, мы определили стратегию $\fOpt$ и соответствующий гарантированный выигрыш первого игрока $\lowerBound{p}$ для любых $p \in [0, 1], \, \Co \in (0, 1)$.
...
\section{Значение игры $\mathbf{G^m_\infty(p)}$}\label{ch1:game-value}
