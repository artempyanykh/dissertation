...
% Предположим, что игрок~1 обладает точной информацией о функции
% выигрыша в данной игре. Игрок~2 данной информацией не обладает. Он знает, что
% настоящая функция выигрыша является одной из $\kappa$ возможных альтернатив.
% По-другому, имеется некоторое множество $S$ возможных состояний, и каждому
% состоянию $s \in S$ ставится в соответствие некоторая игра $G_s$. Игрок~2
% каждому состоянию $s$ приписывает вероятность $p_s$ того, что $s$ является
% истинным состоянием. Оба игрока знают стратегии, доступные противнику. Кроме
% того, игрок~2 знает, что игрок~1 обладает точной информацией о том, какая из
% альтернативных игр разыгрывается, а игрок~1 знает какие вероятности второй игрок
% приписывает альтернативам.

% Подчеркнем, что только одна из игр $G_1, \ldots, G_\kappa$ повторяется; игрок~1
% знает точно какая именно, в то время как игрок~2 знает только вероятностное
% распределение $\po = (p^0_1, p^0_2, \ldots, p^0_\kappa)$ на множестве возможных
% альтернатив. На рисунке~\ref{fig:ch1:game_structure} изображена структура
% повторяющейся игры с точки зрения второго игрока.
...
% Обозначим эту игру $G(\po)$. Предположим, что она имеет значение, а $\sigma$ и
% $\tau$ "--- оптимальные стратегии.\footnote{Точные определения значения игры и
%   оптимальных стратегий будут даны позднее.}
