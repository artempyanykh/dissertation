...
В силу неравенств (\ref{eq:lowerBound:inequalities}) и формулы
...
\section{Значение игры и динамика апостериорных вероятностей}
...
Пусть $p \in P$. Обозначим через $p^t$ апостериорную вероятность состояния $H$
...
поглощающими. Они соответствуют моменту игры, в который поисходит полное
...
\begin{equation*}
  ...
  % \xi = \min \{0 \leq t: p^t = p_0 \lor p^t = p_{2m}\}
\end{equation*}
...
Обозначим через $\pi^l_i,\ \pi^r_i,\ i \in \overline{1, 2m-1}$ вероятности
...
\begin{proposition}
  ...
  шагов. Её ожидаемая продолжительность выражается формулами
  \begin{equation*} 
...
Done:
\begin{enumerate}
\item Написать по поводу рекурсивной структуры игры (в разделе про оценку
  снизу?)
...
\cite{pyanykh14, pyanykh16:discr:eng, pyanykh16:discr:ru,
