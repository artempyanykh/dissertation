...
\begin{equation}\label{ch3:eq:subdiff-props}
  v^* \in \partial f(v)
  \iff v = \arg\!\min_w \left( w v^* - f(w) \right)
  \iff v \in \partial f^*(v^*) .
\end{equation}
...
\begin{proof}
  ...
  \begin{equation}\label{ch3:eq:g1-def}
    ...
    \rangle,
  \end{equation}
  где функция $t(x, y)$ определена в~(\ref{ch3:eq:trans-mech}).
...
\begin{lemma}
  ...
  Функции $f$ и $Q$\textnormal{,} определенные в~\eqref{ch3:eq:f} и \eqref{ch3:eq:Q(u)}\textnormal{,} принимают значения в $[0, 1]$ и удовлетворяет~\eqref{ch3:eq:parametr-prop:f-incr} "--- \eqref{ch3:eq:parametr-prop:f-Q-eq}\textnormal{,} т.е. параметризуют некоторое распределение $\sigma_1$.
\end{lemma}
\begin{proof}
...
\begin{theorem}\label{ch3:thm:V-bound}
  При любом $p \in [0, 1]$ для гарантированного выигрыша первого игрока в игре $G_{n+1}(p)$ справедлива оценка $\LV{n+1}(p) \geqslant K^*(p)$.
\end{theorem}
\begin{proof}
...
В~\cite{demeyer02} показано, что $\psi^*_n(z) = \frac{1}{\sqrt{n}} W_n(z) = P(S_n > z)$.
Отсюда получаем, что
\[
  W'_n(z)
  = \psi^*_n \left( \frac{z}{\sqrt{n}} \right) 
