...
\begin{lemma}
  ...
  \begin{equation}
    V_{1,n+1}(p) \geqslant \sup_{\sigma_1} \inf_{y} g_1(p, \sigma_1, y)
    + \E_{\Pi[p, \sigma_1]} V_{1,n}\left( p(X) \right).
  \end{equation}
\end{lemma}
...
\begin{lemma}\label{ch3:lem:low-bound-dual:general}
  ...
  \begin{equation}\label{ch3:eq:low-bound-dual:general}
    W_{2,n+1}(z) \geqslant \sup_{\tau_1} \inf_x
    W_{2,n}(z - g^H_1(x, \tau_1) + g_1^L(x, \tau_1)) - g^L_1(x, \tau_1).
  \end{equation}
\end{lemma}
...
\begin{proof}
  ...
  \begin{equation}\label{ch3:eq:g1-def}
    g_1(p, \sigma_1, y) = \E_{\Pi[p, \sigma_1]} \langle
    (\Ind{s = H}, 1), t(x, y)
    \rangle,
  \end{equation}
...
\begin{lemma}
  Для любого $p \in [0, 1]$ выполняется неравенство
  \[
    V_{1,n+1}(p) \geqslant \sup_{(f, \; Q)} \inf_{y} \Ff{y},
  \]
...
\begin{lemma}\label{ch3:lem:W-low-bound:parameterized}
  ...
  \begin{equation*}
    W_{2,n+1}(z) \geqslant \sup_h \inf_{x} \Gf[x][h].
  \end{equation*}
\end{lemma}
...
\begin{proof}
  ...
  \begin{multline*}
    \Gf[x][h] =
    W_{2,n}(z + 1) - \int_0^1 \left( \DCo x + \Co h(u) \right) \di u \geqslant \\
  ...
  \begin{multline}\label{ch3:eq:g+-}
    \Gf[x][h] = W_{2,n}(z + 1 - \alpha^- - \alpha^+) \enskip - \\
  ...
  \begin{multline*}
    \Gf[x][h] \geqslant W_{2,n}(z + 1 - 2\alpha)
    - \int_\alpha^1 \biggl( \DCo h(\alpha) + \Co h(u) \biggr) \di u \; + \\
    + \int_0^\alpha \biggl( \Co h(\alpha) + \DCo h(u) \biggr) \di u =
    \Gf[h(\alpha)][h] = \Psi(h).
  \end{multline*}
